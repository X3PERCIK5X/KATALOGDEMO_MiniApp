...
function parseCsvRecords(csvText) {
  ...
  for (let i = 0; i < text.length; i += 1) {
    ...
    if (char === '\n' && !inQuotes) {
      if (String(current || '').trim()) records.push(current);
      current = '';
      continue;
    }
    current += char;
...
export function parseCsvRows(csvText) {
  ...
  const headers = parseCsvLine(records[0], delimiter).map((value) => String(value || '').trim());
  if (!headers.some(Boolean)) return [];
  const rows = [];
  for (let index = 1; index < records.length; index += 1) {
    const cells = parseCsvLine(records[index], delimiter);
    const row = {};
