const ALIAS_SETS = {
  title: [
    ...
    'mark', 'model', 'product_mark', 'наименование', 'название', 'название_товара', 'наименование_товара', 'товар', 'товарное_название', 'товарное_наименование', 'марка', 'модель',
  ],
  sku: [
  ...
  description: [
    'description', 'details', 'detail', 'short_description', 'text', 'content', 'описание', 'описание_товара', 'краткое_описание', 'полное_описание', 'текст', 'comment', 'комментарий',
  ],
  characteristics: [
...
export function mapImportFieldName(value) {
  ...
  if ((/^old_/.test(base) || /_old$/.test(base) || /old/.test(base)) && /price|cost|цен/.test(base)) return 'old_price';
  if (/price.*old|cost.*old|цена.*стара|цена.*до.*скид|старая.*цена/.test(base)) return 'old_price';
