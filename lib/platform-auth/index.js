...
function parseVkLaunchParams(rawLaunchParams) {
  ...
  try {
    ...
    return { ok: true, raw: decodedRaw, params };
  } catch {
    return { ok: false, error: 'VK_LAUNCH_PARAMS_INVALID', raw, params: new URLSearchParams() };
  }
}
...
function verifyVkLaunchParams(rawLaunchParams, { vkAppId = '', vkAppSecret = '' } = {}) {
  ...
  for (const param of raw.split('&')) {
    ...
    const value = separatorIndex >= 0 ? normalized.slice(separatorIndex + 1) : '';
  ...
  if (!queryParams.length) return { ok: false, error: 'VK_LAUNCH_PARAMS_INVALID' };

  const ordered = queryParams
    .sort((a, b) => a.key.localeCompare(b.key))
    .reduce((acc, entry, index) => {
      return acc + (index === 0 ? '' : '&') + `${entry.key}=${encodeURIComponent(entry.value)}`;
    }, '');
