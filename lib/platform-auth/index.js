...
function verifyVkLaunchParams(rawLaunchParams, { vkAppId = '', vkAppSecret = '' } = {}) {
  ...
  const vkPairs = rawPairs
    .filter((part) => {
      const [rawKey = ''] = part.split('=', 1);
      return rawKey.startsWith('vk_');
    })
    .sort((a, b) => {
  ...
  if (!vkPairs.length) return { ok: false, error: 'VK_LAUNCH_PARAMS_INVALID' };

  const ordered = vkPairs.join('&');
